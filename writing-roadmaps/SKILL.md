...
description: Use when a task is too large for one spec/plan — a multi-week migration, a multi-release rollout, a phased rewrite — and the user asks for a "roadmap"/"дорожная карта", wants progress checkpoints, or wants each step tracked as a separate ticket. Not for a single-spec-sized feature; use brainstorming/writing-plans directly for those.
---
...
**REQUIRED BACKGROUND:** `superpowers:brainstorming` — this skill reuses its dialogue discipline (one question at a time, propose 2-3 approaches, incremental section-by-section approval) and its native-Plan-Mode requirement. Read it first; this skill only covers what's different for a multi-step decomposition. Storage/naming (where the roadmap, the feature-level PRD, and each step's own PRD/spec/plan live) is governed by `roadmap-conventions` — read it before writing anything to disk.
...
Concretely this usually means: additive changes (new module, new table, dual-write) before destructive ones (removing the old path), and "read path still works during the transition" before "write path switches over." See `docs/care-jira-source-of-truth/` in a project using this pattern for a worked example (sidecar table → dual-write → read cutover → write cutover → old-path cleanup, five steps, each independently shippable).
...
3. The **feature-level PRD** is written after the roadmap, not before: don't look for it or write it yet at this step. Once the roadmap decomposition is approved (step 7) and saved (step 8), write the feature-level PRD (`roadmap-conventions`, format per `writing-prd`) at `docs/<feature-slug>/YYYY-MM-DD-<feature-slug>-prd.md`, based on the now-decided roadmap — a roadmap is never finalized without a linked feature-level PRD, but the PRD documents that decision rather than preceding it.
...
| Dependencies | `(depends: <id>...)` only when truly needed — lets `executing-roadmap-checkpoints` offer parallel checkpoints instead of forcing document order |
...
- **Batch-asking all clarifying questions in one message.** Breaks the `brainstorming` dialogue discipline this skill inherits — one question at a time.
- **Finalizing the roadmap with no linked feature-level PRD.** `roadmap-conventions` requires one — write it with `writing-prd` right after the roadmap is saved, don't skip it because the roadmap "already explains the problem."
...
- **Tagging every checkpoint as depending on the previous one "to be safe."** Defeats the purpose of `(depends: ...)` — only tag a real dependency; checkpoints with no real dependency should be tag-free so `executing-roadmap-checkpoints` can actually offer them in parallel.
