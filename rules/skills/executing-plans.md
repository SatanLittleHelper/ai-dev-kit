...
**REQUIRED SUB-SKILL:** Invoke `superpowers:executing-plans` — it owns the actual execution/review-checkpoint process. This file is a required layer on top: how mechanically to trust the plan, and what not to do around commits.
...
- **No `git add`/`git commit`/PR creation without a direct request from the user** — see `rules/base/git-and-commits.md` for the general rule this instance follows.
