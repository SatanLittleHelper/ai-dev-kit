...
- **No explicit "negated-flag" class branch for any boolean state:** for any boolean condition (`selected`, `active`, `disabled`, `expanded`, ...), the base classes already are the `false` look — add an override only for the `true` branch (`[class.foo]="flag()"` or a ternary that swaps just the differing classes), never a separate branch keyed on the negated flag (`!flag()`) that just re-states the default. The `false` state is the default, not a state to style separately.
  - **Exception:** only when the component's base/idle template classes don't match *either* boolean value's look on their own — i.e. `true` and `false` are both distinct overrides of a neutral base, not "default vs. one override." In that case write both branches explicitly; there's no default to fall back to.
