...
4. **Ask before committing.** Per `repo-workflow-conventions`, `git commit` only runs on explicit user request — never automatically here. Present the diff and ask whether to commit. If the user says yes, follow `repo-workflow-conventions` for message shape (past tense, one line, ticket prefix from `.claude/dev-conventions.json` if the project tracks one).
...
| Committing `skills-lock.json` changes automatically after the update | Ask first, per `repo-workflow-conventions` — commit only on explicit request |
