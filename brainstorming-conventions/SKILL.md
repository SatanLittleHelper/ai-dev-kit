---
name: brainstorming-conventions
description: Use when turning an idea or requirement into a design before implementation — the same triggering moment as superpowers:brainstorming.
---
...
**REQUIRED SUB-SKILL:** Invoke `superpowers:brainstorming` — it owns the actual design process (spike/bounded/architectural classification, clarifying questions, approaches). This skill is a required pre/post layer around it: native Plan Mode wiring, trigger-phrase recognition, and where the resulting spec lives.
...
After the design is approved, hand off to the writing-plans layer (`writing-plans-conventions`, which itself requires `superpowers:writing-plans`) — not `superpowers:writing-plans` directly. Do not start implementation before the design is agreed on, when the task genuinely requires an architectural choice.
