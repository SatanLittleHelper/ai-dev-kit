---
name: batch-plan-validate
description: >-
  Use when the user needs several plans or specs written and validated one at a time —
  triggers include "напиши несколько планов" / "write several plans", "напиши план на
  каждую задачу" / "write a plan for each task", "несколько спек" / "several specs",
  "провалидировать по очереди" / "validate one by one", "батч планов" / "batch of plans",
...
1. **Roadmap file** — parse top-level unchecked `- [ ]` checklist items as queue entries; task id = the item's label/number (e.g. `B4`, `C`); step-slug = the slug for that task's `docs/<feature-slug>/steps/<step-slug>/` folder (`roadmap-conventions`). If the step folder already has a `-design.md` or `-prd.md`, pass it as context to the drafting skill instead of redrafting it.
...
For each queue task still `queued` or `drafted`, in order: if the task's step folder has no PRD yet, run `writing-prd` first, inline, saving to `docs/<feature-slug>/steps/<step-slug>/YYYY-MM-DD-<step-slug>-prd.md`; then run the chosen drafting skill **inline** (not as a subagent — you need to intercept its blocking-question points), using its normal research subagents (`Explore`/`Plan`) as usual. Save the draft into the same step folder (`roadmap-conventions` naming: `writing-plans` → `YYYY-MM-DD-<ticket>-<name>.md`; `brainstorming` → `YYYY-MM-DD-<step-slug>-design.md`) via plain `Write` — do not enter native Plan Mode for this phase, since Plan Mode would force `ExitPlanMode`/`AskUserQuestion` at the end of every turn and you need to move through the whole queue in one pass.
...
- That re-invoking the skill on the same source will resume validation from the next batch automatically (Phase 1/1.5 are skipped for already-drafted tasks).
...
- **Skipping the step-level PRD and drafting a plan/design straight away.** `roadmap-conventions` requires a PRD in every task's `steps/<step-slug>/` folder — write it first with `writing-prd` unless one already exists there.
