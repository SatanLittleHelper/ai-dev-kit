...
For updating the rules submodule (`.claude/ai-dev-kit`) and any generated `AGENTS.md`, see `update-project-rules` instead — a separate skill, deliberately not folded into this one.
...
| Also updating `.claude/ai-dev-kit`/`AGENTS.md` here because they come from the same repo | That's `update-project-rules`'s job — keep the two separate, don't fold rules-updating logic back into this skill |
